...
vm_exit proc
  ...
  sub rsp, 512  ;Intel手册有介绍必须是512字节的内存空间
  fxsave64 [rsp]
  mov rdx, rsp ;param2, pointer to fxsave
...
stop_virtualization:
  ...
  ; context.
  ; 我们将弄脏这些寄存器以便设置栈，因此我们需要在使用它们之前存储和恢复它们。
  ; 另请注意，我们没有为陷阱帧分配任何栈空间，因为我们可以重用为guest上下文分配的空间。
  ; 以下开始填充_MACHINE_FRAME结构
  push rax
  push rdx
  push rbp
  lea rbp, [rsp + 38h]
